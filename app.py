...
def predict():
    ...
    if isinstance(data, list):
        ...
        results = [{
            ...
            "prediction": "Paid Back" if preds[i] == 1 else "Defaulted",
    ...
    # Single Processing
        ...
        log_df = pd.DataFrame([{"timestamp": ts, "prediction": "Paid Back" if pred == 1 else "Defaulted", "confidence": round(prob, 4)}])
        log_df.to_csv(LOG_FILE, mode='a', header=not os.path.exists(LOG_FILE), index=False)
        
        return jsonify({
            "prediction": "Paid Back" if pred == 1 else "Defaulted",
